...
//
    ...
    if (isset($_GET['action']) and $_GET['action'] === 'rewrite' and isset($_SESSION['form'])) {
        ...
        $form = [  //配列$formは連想配列にnameを使うと宣言
            ...
            'nenryou' => '',
            'uriage' => '',
            'match' => ''  //membersテーブルのmember_idと
            //           nichibetu.phpで入力するmember_idの照合に使う
        ];  //空のフォームの配列
    }
    ...
    if (isset($_SESSION['name']) and isset($_SESSION['id']) and isset($_SESSION['member_id'])) {//login.phpから'name'をセッションで受け取る
		$name = $_SESSION['name'];
        $id = $_SESSION['id'];
        $m_member_id = $_SESSION['member_id'];//membersテーブルのmember_id
	} else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {  //もし送信ボタンが押されたら
        ...
        $form_member_id = $form['member_id'];//$form['member_id']を$from_member_idにする
        $form_member_id = (int) $form_member_id;//$form_member_idのstr型をint型にする!

        if ($form_member_id !== $m_member_id) {
            $error['match'] = 'failed';
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <dl>
                ...
                <dd>
                    ...
                    <?php endif;?>
                    <?php if (isset($error['notmatchlen']) and $error['notmatchlen'] === 'notmatchlen'):?>
                        <p class="error">* 西暦月は6桁で入力してください</p>
                    <?php endif;?>
                </dd>
